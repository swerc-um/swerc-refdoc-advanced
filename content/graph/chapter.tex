...
\section{Fundamentals}
    ...
    \textbf{Number of paths of a fixed length}\\
	Given the adjacency matrix $G$ of a graph, the number of paths from node $a$ to node $b$ using exactly $k$ edges equals $G^k[a][b]$.

	\textbf{Shortest paths of a fixed length}\\
	$C[i][j] = \min_{k=1}^N \; A[i][k] + B[k][j]$.  
	Identity matrix where the main diagonal values are zeros and all other values are infinity values.  
	Shortest path length from node $a$ to node $b$ using exactly $k$ edges equals $G^k[a][b]$.  
	Be careful with duplicate edges.

	\textbf{Generalization of the problems for paths with length up to $k$}\\
	...
	The number of paths between $i$ and $j$ with at most $k$ edges is the same as the number of paths between $i$ and $j'$ with exactly $k+1$ edges.  
	...
	\textbf{Eulerian subgraphs}\\
In a undirected graph, we consider subgraphs that have all nodes of the original graph and some of its edges. A subgraph is called Eulerian if each node has even degree.
Number of Eulerian subgraphs = $2^{n-m+cc}$.

	\textbf{Tarjan criterion}\\
